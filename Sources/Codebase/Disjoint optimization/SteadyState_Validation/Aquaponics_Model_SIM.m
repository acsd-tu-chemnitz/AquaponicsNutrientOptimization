function [xdot,pH] = Aquaponics_Model_SIM(x,u,z,param) 
    
    xdot   = zeros(param.number_of_states,1);
    ...
    fun = @(y)(x(INDEX.x_c_BF.Alkalinity)- x(INDEX.x_c_BF.Total_Carbon)*(param.K_1*y+2*param.K_1*param.K_2)/(y^2+param.K_1*y+param.K_1*param.K_2) - x(INDEX.x_c_FT.Phosphorus)*(param.K_5)/(y+param.K_5))^2;
    sol = fsolve(fun,10^-1);
    c_H_BF = sol;
    fun = @(y)(x(INDEX.x_c_FT.Alkalinity)- x(INDEX.x_c_FT.Total_Carbon)*(param.K_1*y+2*param.K_1*param.K_2)/(y^2+param.K_1*y+param.K_1*param.K_2) - x(INDEX.x_c_FT.Phosphorus)*(param.K_5)/(y+param.K_5))^2;
    sol = fsolve(fun,10^-1);
    c_H_FT = sol;
    fun = @(y)(x(INDEX.x_c_BT.Alkalinity)- x(INDEX.x_c_FT.Total_Carbon)*(param.K_1*y+2*param.K_1*param.K_2)/(y^2+param.K_1*y+param.K_1*param.K_2) - x(INDEX.x_c_BT.Phosphorus)*(param.K_5)/(y+param.K_5))^2;
    sol = fsolve(fun,10^-0);
    c_H_BT = sol;

    pH = [c_H_FT; c_H_BF; c_H_BT];
    c_ammonium_BF = x(INDEX.x_c_BF.Total_Ammonium);
    
    c_CO2         = x(INDEX.x_c_BF.Total_Carbon)*c_H_BF^2/(c_H_BF^2 + param.K_1*c_H_BF + param.K_1*param.K_2);
    ...
    for i = 1:param.number_of_nutrients
        ...
        if(param.is_set_by_feed_content(i) == 1)            
            xdot(INDEX.x_c_FT.(param.nutrient_identifiers(i))) = 1/(param.V_FT+param.V_BF) * ( param.c_in(i) * u_in_FT + param.Feed_to_Nut(i,1) * u_feed +  param.base_to_nutrient(i,:) * u(INDEX.u_Base.(param.base_identifiers(1)):INDEX.u_Base.(param.base_identifiers(end))) -u_in_FT * x(INDEX.x_c_FT.(param.nutrient_identifiers(i))));
        elseif(strcmp(param.nutrient_identifiers(i),"Total_Carbon")==1)
            xdot(INDEX.x_c_FT.(param.nutrient_identifiers(i))) = 1/(param.V_FT) * ( param.c_in(i) * u_in_FT + x(INDEX.x_c_BF.("Total_Carbon"))*u_circ_BF + param.Feed_to_Nut(i,1) * u_feed - x(INDEX.x_c_FT.("Total_Carbon"))*u_in_FT - x(INDEX.x_c_FT.("Total_Carbon"))*u_circ_BF );
        elseif(strcmp(param.nutrient_identifiers(i),"Alkalinity")==1)
            xdot(INDEX.x_c_FT.(param.nutrient_identifiers(i))) = 1/param.V_FT * ( param.c_in(i) * u_in_FT +  param.Feed_to_Nut(i,1) * u_feed + param.base_to_nutrient(i,:) * u(INDEX.u_Base.(param.base_identifiers(1)):INDEX.u_Base.(param.base_identifiers(end))) + u_circ_BF * x(INDEX.x_c_BF.(param.nutrient_identifiers(i)))  ...
        ...
        elseif(strcmp(param.nutrient_identifiers(i),"Hydrogen")~=1)
            xdot(INDEX.x_c_FT.(param.nutrient_identifiers(i))) = 1/param.V_FT * ( param.c_in(i) * u_in_FT +  param.Feed_to_Nut(i,1) * u_feed + u_circ_BF * x(INDEX.x_c_BF.(param.nutrient_identifiers(i))) +  param.base_to_nutrient(i,:) * u(INDEX.u_Base.(param.base_identifiers(1)):INDEX.u_Base.(param.base_identifiers(end))) ...
                                                             - u_circ_BF * x(INDEX.x_c_FT.(param.nutrient_identifiers(i))) - u_in_FT * x(INDEX.x_c_FT.(param.nutrient_identifiers(i))) );       
        end
