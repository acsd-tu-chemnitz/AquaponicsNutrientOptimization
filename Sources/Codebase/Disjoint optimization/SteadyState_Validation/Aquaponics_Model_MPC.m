function xdot = Aquaponics_Model_MPC(x,u,p,param) 
    ...
    for i = 1:param.number_of_nutrients
        ...
        if(param.is_set_by_feed_content(i) == 1)            
            xdot(INDEX.x_c_FT.(param.nutrient_identifiers(i))) = 1/(param.V_FT+param.V_BF) * ( param.c_in(i) * u_in_FT + param.Feed_to_Nut(i,1) * q_feed +  param.base_to_nutrient(i,:) * u(INDEX.u_Base.(param.base_identifiers(1)):INDEX.u_Base.(param.base_identifiers(end))) -u_in_FT * x(INDEX.x_c_FT.(param.nutrient_identifiers(i))));
        elseif(strcmp(param.nutrient_identifiers(i),"Total_Carbon")==1)
            xdot(INDEX.x_c_FT.(param.nutrient_identifiers(i))) = 1/(param.V_FT) * ( param.c_in(i) * u_in_FT + x(INDEX.x_c_BF.("Total_Carbon"))*u_circ_BF + param.Feed_to_Nut(i,1) * q_feed - x(INDEX.x_c_FT.("Total_Carbon"))*u_in_FT - x(INDEX.x_c_FT.("Total_Carbon"))*u_circ_BF );
        elseif(strcmp(param.nutrient_identifiers(i),"Alkalinity")==1)
            xdot(INDEX.x_c_FT.(param.nutrient_identifiers(i))) = 1/param.V_FT * ( param.c_in(i) * u_in_FT +  param.Feed_to_Nut(i,1) * q_feed + param.base_to_nutrient(i,:) * u(INDEX.u_Base.(param.base_identifiers(1)):INDEX.u_Base.(param.base_identifiers(end))) + u_circ_BF * x(INDEX.x_c_BF.(param.nutrient_identifiers(i)))  ...
        ...
        elseif(strcmp(param.nutrient_identifiers(i),"Hydrogen")~=1)
            xdot(INDEX.x_c_FT.(param.nutrient_identifiers(i))) = 1/param.V_FT * ( param.c_in(i) * u_in_FT +  param.Feed_to_Nut(i,1) * q_feed + u_circ_BF * x(INDEX.x_c_BF.(param.nutrient_identifiers(i))) +  param.base_to_nutrient(i,:) * u(INDEX.u_Base.(param.base_identifiers(1)):INDEX.u_Base.(param.base_identifiers(end))) ...
                                                             - u_circ_BF * x(INDEX.x_c_FT.(param.nutrient_identifiers(i))) - u_in_FT * x(INDEX.x_c_FT.(param.nutrient_identifiers(i))) );       
        end
