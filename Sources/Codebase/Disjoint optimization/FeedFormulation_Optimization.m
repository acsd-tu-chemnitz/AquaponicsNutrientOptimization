...
% Note: Weight for the maximization of nutrients as presented in"Optimization of fish feed formulation and pH regulation in on-demand coupled aquaponic systems: A model-based framework for sustainable nutrient and water management"
% can be found on lines 130-143, the respective weights for the mimization of nutrients can be found on lines 162-166
...
for i=1:length(constraints_str)
    if((strcmp(constraint_bounds.("Restriction Type")(i),'Ratio')==1 || strcmp(constraint_bounds.("Restriction Type")(i),'ratio')==1) && ~isnan(constraint_bounds.("Value")(i)))
        if strcmp(constraint_bounds.("Unit")(i),"g/MJ")==1
            DP_DE_ratio=constraint_bounds.("Value")(i)/param.MJ_to_kcal;
        end
...
% Weights far maximizing nutrient ins the final feed
% K-max feed
weight_max = [100;0;0;0];

% P-Max feed
%weight_max = [0;0;100;0];

% Mg-max feed
%weight_max = [0;100;0;0];

% KPMg-max feed
%weight_max = [40;40;20;0];
...
A_con(con_counter+1,1:height(Feed_Par)) = Feed_Par.("Crude  Protein (%)")' - param.CPGE_min*Feed_Par.("Gross Energy -MJ (MJ/kg)")';
A_con(con_counter+2,1:height(Feed_Par)) = - Feed_Par.("Crude  Protein (%)")' + param.CPGE_max*Feed_Par.("Gross Energy -MJ (MJ/kg)")';
...
% Tracking of CPGE ratio
w_CPGE = 8;
quadratic_objective(2*height(Feed_Par)+3,2*height(Feed_Par)+3) = w_CPGE*10000/(40*param.CPGE_max)^2;
quadratic_objective(2*height(Feed_Par)+3,2*height(Feed_Par)+4) = w_CPGE*10000/(40*param.CPGE_max)^2 * (-1)*param.CPGE_max;
quadratic_objective(2*height(Feed_Par)+4,2*height(Feed_Par)+3) = w_CPGE*10000/(40*param.CPGE_max)^2 * (-1)*param.CPGE_max;
quadratic_objective(2*height(Feed_Par)+4,2*height(Feed_Par)+4) = w_CPGE*10000/(40*param.CPGE_max)^2 * param.CPGE_max^2;

% Tracking of CaPO ratio
quadratic_objective(2*height(Feed_Par)+5,2*height(Feed_Par)+5) = 10000/(param.Ca_PO_ratio)^2 * param.Ca_PO_ratio^2;
quadratic_objective(2*height(Feed_Par)+5,2*height(Feed_Par)+6) = 10000/(param.Ca_PO_ratio)^2 * (-1)*param.Ca_PO_ratio;
quadratic_objective(2*height(Feed_Par)+6,2*height(Feed_Par)+5) = 10000/(param.Ca_PO_ratio)^2 * (-1)*param.Ca_PO_ratio;
quadratic_objective(2*height(Feed_Par)+6,2*height(Feed_Par)+6) = 10000/(param.Ca_PO_ratio)^2;
...
for i = 1:num_nut
    ...
    res_cont_nut_sol = u_sol(1:height(Feed_Par))'*Feed_Par.(dietary_nutrients(i));
    opt_results(i*2)= res_cont_nut_sol;
end

CO2_eq = u_sol(1:height(Feed_Par))'*Feed_Par.("Global warming - Including LUC & Peat (kg CO2eq/t)")

CP = Feed_Par.("Crude  Protein (%)")'*u_sol(1:height(Feed_Par));
GE = Feed_Par.("Gross Energy -MJ (MJ/kg)")'*u_sol(1:height(Feed_Par));
DP = Feed_Par.("Dig CP -fish (%)")'*u_sol(1:height(Feed_Par));
DE = Feed_Par.("Dig GE (DE) - fish (kcal)")'*u_sol(1:height(Feed_Par));

CPGE_ratio = CP/GE
DPDE_ratio = DP/DE *param.MJ_to_kcal
CaPO_ratio = Feed_Par.("Calcium (%)")'*u_sol(1:height(Feed_Par))/(Feed_Par.("Phosphorus (%)")'*u_sol(1:height(Feed_Par)))
